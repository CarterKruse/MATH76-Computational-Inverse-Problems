...
%% Model 2) Convolution Matrix

% Let N = 100.
N = 100;

%% Set-Up
% Generate a matrix 'A' in R^{N x N} such that 'A' is a matrix that
% represents a Gaussian blur.

% Factor inversely proportional to standard deviation of Gaussian (normal 
...
for i = 1:N
    A(i,:) = circshift(blur_vector, i - 1, 1); % Build columns of 'A'.
    if i < N / 2
        A(i, N / 2 + i : N) = 0;
    elseif i > N / 2
        A(i, 1 : N / 2 - i) = 0;
...
lgd = legend('X', 'Least Squares', 'Truncated SVD', 'L2', 'L1', 'location', 'northwest');
